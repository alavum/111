...
export function initSqlite(dbFile = path.join("data", "app.db")) {
  try {
    ...
    try {
      ...
      if (count === 0) {
        const newsJson = safeParseJson(
          path.join("data", "news-articles.json"),
          [],
        );
        const insert = db.prepare(
          `INSERT INTO news (id,title,content,author,date,published,image,excerpt,category,slug) VALUES (@id,@title,@content,@author,@date,@published,@image,@excerpt,@category,@slug)`,
        );
        const insertMany = db.transaction((items: any[]) => {
          for (const it of items) {
            insert.run({
              ...
              excerpt:
                it.excerpt || (it.content || "").substring(0, 150) + "...",
      ...
      const ruleCount =
        db.prepare(`SELECT COUNT(1) as c FROM rules`).get()?.c || 0;
      if (ruleCount === 0) {
        const rulesJson = safeParseJson(path.join("data", "rules.json"), null);
        if (rulesJson) {
          db.prepare(
            `INSERT INTO rules (id,title,content,lastUpdated) VALUES (1,@title,@content,@lastUpdated)`,
          ).run({
      ...
      const privacyCount =
        db.prepare(`SELECT COUNT(1) as c FROM privacy`).get()?.c || 0;
      if (privacyCount === 0) {
        const privacyJson = safeParseJson(
          path.join("data", "privacy.json"),
          null,
        );
        if (privacyJson) {
          db.prepare(
            `INSERT INTO privacy (id,title,content,lastUpdated) VALUES (1,@title,@content,@lastUpdated)`,
          ).run({
      ...
      const termsCount =
        db.prepare(`SELECT COUNT(1) as c FROM terms`).get()?.c || 0;
      if (termsCount === 0) {
        const termsJson = safeParseJson(path.join("data", "terms.json"), null);
        if (termsJson) {
          db.prepare(
            `INSERT INTO terms (id,title,content,lastUpdated) VALUES (1,@title,@content,@lastUpdated)`,
          ).run({
    ...
    console.warn(
      "better-sqlite3 not available, falling back to JSON file storage",
    );
...
export function getNewsByIdOrSlug(param: string) {
  ...
  if (!row) {
    row = db
      .prepare(`SELECT * FROM news WHERE slug = ? AND published=1 LIMIT 1`)
      .get(param);
  }
  return row;
}
export function createNewsRow(data: any) {
  ...
  const stmt = db.prepare(
    `INSERT INTO news (title,content,author,date,published,image,excerpt,category,slug) VALUES (@title,@content,@author,@date,@published,@image,@excerpt,@category,@slug)`,
  );
  const info = stmt.run({
    title: data.title,
...
export function updateRulesRow(data: any) {
  ...
  if (existing) {
    db.prepare(
      `UPDATE rules SET title = @title, content = @content, lastUpdated = @lastUpdated WHERE id = @id`,
    ).run({
      title: data.title,
      content: data.content,
  ...
  const info = db
    .prepare(
      `INSERT INTO rules (title,content,lastUpdated) VALUES (@title,@content,@lastUpdated)`,
    )
    .run({
      title: data.title,
      content: data.content,
...
export function updatePrivacyRow(data: any) {
  ...
  if (existing) {
    db.prepare(
      `UPDATE privacy SET title = @title, content = @content, lastUpdated = @lastUpdated WHERE id = @id`,
    ).run({
      title: data.title,
      content: data.content,
      lastUpdated: new Date().toISOString(),
  ...
  const info = db
    .prepare(
      `INSERT INTO privacy (title,content,lastUpdated) VALUES (@title,@content,@lastUpdated)`,
    )
    .run({
      title: data.title,
      content: data.content,
  ...
  return db
    .prepare(`SELECT * FROM privacy WHERE id = ?`)
    .get(info.lastInsertRowid);
}
...
export function updateTermsRow(data: any) {
  ...
  if (existing) {
    db.prepare(
      `UPDATE terms SET title = @title, content = @content, lastUpdated = @lastUpdated WHERE id = @id`,
    ).run({
      title: data.title,
      content: data.content,
      lastUpdated: new Date().toISOString(),
  ...
  const info = db
    .prepare(
      `INSERT INTO terms (title,content,lastUpdated) VALUES (@title,@content,@lastUpdated)`,
    )
    .run({
      title: data.title,
      content: data.content,
